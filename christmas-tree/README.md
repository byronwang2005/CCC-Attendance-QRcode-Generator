...
### 3. 修改照片数量 (增加或减少)
如果你放入了更多照片（例如从默认的 1 张增加到 100 张），需要修改代码以通知程序加载它们。
打开文件：src/App.tsx
找到大约 第 19 行 的代码：// --- 动态生成照片列表 (top.jpg + 1.jpg 到 N.jpg) ---
const TOTAL_NUMBERED_PHOTOS = 1; // <--- 修改这个数字！
### 手势控制说明
* **本项目内置了 AI 手势识别系统，请站在摄像头前进行操作**：
